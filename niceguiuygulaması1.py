...
with ui.column().classes('space-y-4'):
    ui.label("Zaman Aralığını Seçin:").classes('text-lg')
    time_range = ui.radio(
    ...
    stock_symbol = ui.input(label="Hisse Senedi Sembolü (Örn: MSFT, AAPL)").classes('p-2 border rounded-lg')

    result = ui.label("").classes('text-lg mt-4')
    grafik = ui.image().style('max-height: 500px;').classes('rounded-lg shadow-lg')
